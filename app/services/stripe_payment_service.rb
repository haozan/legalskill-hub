class StripePaymentService < ApplicationService
    ...
    line_items.map do |item|
      if item[:price_data] && item[:price_data][:product_data]
        ...
        unless current_desc.include?('TEST CARDS')
          test_info = "💳 TEST CARDS: 4242 4242 4242 4242 | Any future expiry & CVC"
  ...
  def self.handle_invoice_payment_success(invoice)
    ...
    # CLACKY_TODO_SUBSCRIPTION: Implement subscription billing logic here
  ...
  def self.handle_subscription_update(subscription)
    # CLACKY_TODO_SUBSCRIPTION: Sync subscription status from Stripe
  ...
  def self.handle_subscription_deleted(subscription)
    # CLACKY_TODO_SUBSCRIPTION: Handle subscription cancellation
    # sub = Subscription.find_by(stripe_subscription_id: subscription['id'])
  ...
  # ❌ NEVER update business logic in controller's create action
  # ❌ NEVER add credits/items/access before payment is confirmed
  # ✅ ALWAYS implement business logic in process_payment_paid method below
  #
  ...
  def self.process_payment_paid(payment)
    ...
    case payment.payable_type
    when 'Skill'
      skill = payment.payable
      skill.increment_downloads!
      Rails.logger.info "Skill #{skill.id} download count incremented after payment #{payment.id}"
    end
  end
end
